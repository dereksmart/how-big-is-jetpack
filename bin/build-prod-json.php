...
function jetpack_build_reports( $prod_or_dev ) {
	...
	foreach ( $versions as $num => $download_url ) {
		...
		if ( ( preg_match( "/[a-z]/i", $num ) && '7.5-beta' !== $num ) ){
			continue;
		}

		// Download the zip to /tmp
		echo "Try to download zip for $prod_or_dev $num... \n";
		//	$num = 'trunk';
		if ( 'dev' == $prod_or_dev ) {
			if ( strlen( $num ) !== 3 ) {
				continue;
			}
			exec( "wget -O tmp/$prod_or_dev/$num.zip https://github.com/Automattic/jetpack/archive/branch-$num.zip", $out, $return );
		} else if ( 'prod' ) {
			exec( "curl https://downloads.wordpress.org/plugin/jetpack.{$num}.zip --create-dirs -o ./tmp/{$prod_or_dev}/$num.zip", $out, $return );
		}

		if ( $return !== 0 ) {
			exec( "rm ./tmp/{$prod_or_dev}/$num.zip" );
			echo "$prod_or_dev $num not found! \n";
			continue;
		}
