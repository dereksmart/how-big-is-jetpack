...
jQuery( document ).ready( function ( $ ) {
	...
	// Timeline chart
	$.getJSON( "./json/prod/all.json", function( all ) {
		let labels = [],
			sizeZippedData = [],
			sizeUnzippedData = [];
		...
		var pluginSize = document.getElementById( 'pluginSize' ).getContext( '2d' );
		...
		var numFiles = document.getElementById( 'numFiles' ).getContext( '2d' );
		new Chart( numFiles, {
			// The type of chart we want to create
			type: 'line',
		...
		var numLoC = document.getElementById( 'numLoC' ).getContext( '2d' );
		new Chart( numLoC, {
			// The type of chart we want to create
			type: 'line',
